...
const PORTFOLIO_EVENT_ID = 2;

export default function PortfolioSprintRegisterPage() {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            const eventRes = await fetch(`${API_BASE}/api/events/register`, {
                ...
                body: JSON.stringify({ userId: currentUser.id, eventId: PORTFOLIO_EVENT_ID }),
            });
            ...
            <section className="flex flex-1 items-center justify-center px-4 py-16">
                <div className="w-full max-w-xl">
                    <div className="mb-8 border-l-2 border-gold-500 pl-6">
                        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gold-500 mb-2">Technical Workshop</p>
                        <h1 className="text-4xl font-bold tracking-tighter text-white">Portfolio <span className="text-blue-500">Sprint</span></h1>
                    </div>
                    ...
                    <div className="bg-neutral-900/40 p-8 rounded-2xl border border-neutral-800 shadow-2xl backdrop-blur-sm">
                        {submitted ? (
                            <div className="text-center py-10">
                                ...
                                <h3 className="text-xl font-bold text-white mb-2 uppercase tracking-widest">Enrollment_Complete</h3>
                                <p className="text-cool-steel-400 mb-8 text-sm">You are registered for the Portfolio Sprint. Check your profile for dashboard updates.</p>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                {!user && (
                                    <>
                                        ...
                                        <div>
                                            ...
                                            <textarea value={bio} onChange={e => setBio(e.target.value)} rows={2} className="mt-1 w-full bg-neutral-950 border border-neutral-800 px-4 py-2 text-sm rounded-lg outline-none focus:border-blue-500 transition" placeholder="Briefly describe your portfolio goals..." />
                                        </div>

                                        {/* Resume Upload Dropzone */}
                                        <div className="pt-2">
                                            <label className="text-[10px] font-bold uppercase text-neutral-500 tracking-widest block mb-2">Current Resume (Optional)</label>
                                ...
                                <button
                                    type="submit"
                                    disabled={loading}
                                    className="w-full bg-blue-600 py-4 text-xs font-bold uppercase tracking-widest text-white hover:bg-blue-500 transition shadow-lg shadow-red-900/20 disabled:opacity-50 active:scale-95"
                                >
                                    {loading ? "Registering..." : user ? "Confirm Spot" : "Create Account and Register"}
                                </button>
